 ...
 *
 */
public class MessageWebSocketUpDecoder extends ChannelInboundHandlerAdapter {
	protected static final ILogger logger = XLogUtil.logger();

	protected NettyNetConfig config;

	public MessageWebSocketUpDecoder(NettyNetConfig config) {
		this.config = config;
	}
	...
	public void channelRead(ChannelHandlerContext ctx, Object message) throws Exception {
		if (message instanceof HttpRequest) {
			if (logger.isDebugEnabled()) {
				logger.debug("[MessageWebSocketUp Decoder] HttpRequest");
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("[MessageWebSocketUp Decoder] BinaryWebSocketFrame");
			}
			...
			CloseWebSocketFrame closeWebSocketFrame = (CloseWebSocketFrame) message;
			...
			// TODO 这里是测试，如果用需要完善
			if (logger.isDebugEnabled()) {
				logger.debug("[MessageWebSocketUp Decoder] TextWebSocketFrame");
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("[MessageWebSocketUp Decoder] HttpResponse");
			}
			...
			if (logger.isDebugEnabled()) {
				logger.debug("[MessageWebSocketUp Decoder] HttpContent");
			}
			...
			if (logger.isWarnEnabled()) {
				logger.warn("[MessageWebSocketUp Decoder] Other class=" + message.getClass());
			}
		}
	}
}
