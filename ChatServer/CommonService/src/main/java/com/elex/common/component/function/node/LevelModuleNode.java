...
/**
 * 模块节点。如：/sms。这个节点保存每个组节点
 * 
 * @author mausmars
 * 
 */
public class LevelModuleNode extends AFunctionNode {
	public LevelModuleNode(String key) {
		super(key);
	}
	...
	public void zk2localCreate() {
		try {
			...
			for (String childNodeStr : childNodeStrs) {
				if (!context.getNodeFilter().isGroupFilterNode(childNodeStr)) {
					continue;
				}
