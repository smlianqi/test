 ...
 * ignite配置 扩展实体类
 */
public class ScIgnite extends ScIgniteGeneral {
	// 记录索引变量变化前的值
	...
	public static ScIgnite create(byte[] bytes) throws IOException {
        ScIgnite pojo = new ScIgnite();
	...
	@Override
	public ScIgnite cloneEntity(boolean isSaveBefore){
		ScIgnite entity=new ScIgnite();
		...
		entity.discoveryType=this.discoveryType;
		entity.host=this.host;
		entity.port=this.port;
		entity.isClient=this.isClient;
		entity.workDirectory=this.workDirectory;
		entity.swapDirectory=this.swapDirectory;
		return entity;
	}
}