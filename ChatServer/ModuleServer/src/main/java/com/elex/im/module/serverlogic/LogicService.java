 ...
 * 
 */
public class LogicService extends ServerService implements ILogicContext {
	public LogicService() {
		super(FunctionType.logic);
	}

	public LogicService(ServiceManager sm) {
	...
	public void startup(boolean isBlock) {
		if (logger.isInfoEnabled()) {
			logger.info("LogicService startup!");
		}
		// 调用父类启动流程
	...
	public void shutdown() {
		if (logger.isInfoEnabled()) {
			logger.info("LogicService shutdown!");
		}
		// 调用父类关闭流程
		super.shutdown();
	}
